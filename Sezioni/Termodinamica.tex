\section{Termodinamica}
    ...
    \subsection{Gas ideali}
        ...
        \subsection{Entropia}
            ...
            ed avrei \textbf{ottenuto la stessa identica formula}. Morale della favola: $\oint_{A}^B(\frac{dQ}{T})$ \textbf{NON dipende dal percorso che scelgo}. Nota che questo discorso vale solo per i cicli reversibili, per quelli irreversibili si deve fare un po' più di lavoro. Tornando alle reversibili, dato che il percorso non ci interessa possiamo dire che il risultato di $\oint_{A}^B(\frac{dQ}{T})$ è un \textbf{integrando perfetto}, quindi lo possiamo "risolvere" in questo modo:
            \begin{align*}
                \oint_{A}^B(\frac{dQ}{T}) \triangleq S(B)-S(A)
            \end{align*}
            [Small nota di servizio: "$\triangleq$" è "uguale per definizione"] Dove $S$ corrisponde ad un nuova funzione di stato chiamata \textbf{entropia}. Ora un punto molto importante, possiamo dire che:
            \begin{align*}
                \Delta S_{A\rightarrow B}=\oint_{A}^B(\frac{dQ}{T})_{\textcolor{Red}{perRevQualsiasi}}
            \end{align*}
            Concentrati su quel "\textcolor{Red}{per una reversibile qualsiasi}" (perRevQualsiasi): \textbf{la variazione di entropia dallo stato termodinamico A allo stato termodinamico B dipende soltanto dalla posizione di questi 2 punti, ed è completamente \underline{indipendendte dal percorso} che li congiunge}!
