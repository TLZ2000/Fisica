\section{Dinamica}
    ...
    \subsection{Esercizio sulla dinamica}
        ...
        \subsubsection{Posizione di equilibrio}
            Iniziamo calcolando la nuova posizione di equilibrio della molla, ovvero la \textbf{z in cui la molla smette di muoversi}. Per definizione, un oggetto resta fermo quando la sommatoria delle forze che agiscono su di lui è 0, dato che le uniche forze che agiscono sono la forza peso e la forza elastica, possiamo dire che:
            \begin{align*}
                & P = F_{el} && => m*g = -k*(z_{eq}-0) && => \textcolor{Red}{z_{eq} = -\frac{m*g}{k}}
            \end{align*}
            Una cosa utile da fare quando arriviamo alla fine di queste catene di uguaglianze è quella di \textbf{controllare che le unità di misura siano coerenti}, in particolare in questo caso abbiamo:
            \begin{itemize}
                \item $m*g$: forza peso, espressa in $N$, ovvero $\frac{Kg*m}{s^2}$. Possiamo scomporla come:
                \begin{itemize}
                    \item $m$: massa, espressa in Kg;
                    \item $g$: accelerazione (di gravità terrestre), espressa in $\frac{m}{s^2}$
                \end{itemize}
                \item $k$: costante elastica, espressa in $\frac{N}{m}$
            \end{itemize}
            Se mettiamo insieme tutte queste unità di misura, otteniamo effettivamente qualcosa \textbf{espresso in metri}! Quindi ci va bene!

        \subsubsection{Allungamento massimo della molla}
            Qui dobbiamo fare un po' di ragionamenti:
            \begin{enumerate}
                \item all'istante di tempo 0 \textbf{tagliamo il cavo}: la forza peso inizia a poggiare sulla molla, ma la \textbf{forza elastica vale ancora 0} al momento!
                \item col passare del tempo, la molla supererà la posizione $z_{eq}$, da questo punto in poi la \textbf{forza elastica smette di essere rivolta verso il basso e inizia a puntare verso l'alto}, ma dato che la \textbf{costante elastica è la stessa} la molla si comporterà allo stesso modo con cui è scesa!
            \end{enumerate}
